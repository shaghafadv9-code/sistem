...
export default function Login({ onDone }) {
  ...
  useEffect(() => { fetch('/api/brand/public').then(r => r.json()).then(setBrand).catch(() => {}); }, []);
    ...
    <div className="h-full flex items-center justify-center p-4" style={{ background: 'linear-gradient(135deg,#0f1b3d,#1d2a5e 45%,#3b1d6e)' }}>
      <div className="anim-pop w-full flex overflow-hidden rounded-3xl shadow-2xl" style={{ maxWidth: 880, background: 'var(--card)' }}>
        ...
        <form className="flex-1 p-8" onSubmit={submit}>
          ...
          <div className="mt-5 rounded-xl p-3 text-[12px] leading-6" style={{ background: 'var(--bg2)', color: 'var(--ink2)' }}>
            <b>حسابات تجريبية:</b>
            <div className="num">admin / admin123 (مدير) — sara / sara1234 (سكرتيرة)</div>
            <div className="num">accountant / acc12345 — reservations / res12345</div>
          </div>
        </form>
      </div>
    </div>
