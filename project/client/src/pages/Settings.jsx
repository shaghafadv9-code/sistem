...
function DemoTab() {
  ...
  const run = async () => {
    setBusy(true);
    try {
      const r = await api(mode === 'delete' ? '/admin/demo-delete' : '/admin/demo-reset', { method: 'POST', body: { confirm: mode === 'delete' ? 'DELETE' : 'RESET' } });
      toast(mode === 'delete' ? `تم حذف ${r.deleted} سجلًا تجريبيًا` : 'تمت إعادة تعيين البيانات التجريبية');
      setMode(null); setWord(''); load();
    } catch (e) { toast(e.message, 'error'); }
    setBusy(false);
  };
  if (!st) return <Skeleton n={3} />;
  const total = st.tables.reduce((a, t) => a + t.demo, 0);
  const TBL_AR = { payments: 'الدفعات', invoices: 'الفواتير', sales: 'المبيعات', reservations: 'الحجوزات', units: 'الوحدات', buildings: 'المباني', calls: 'الاتصالات', notes: 'الملاحظات', files: 'الملفات', tasks: 'المهام', appointments: 'المواعيد', clients: 'العملاء', brokers: 'الوسطاء' };
  return (
    <div className="space-y-4">
      <div className="card p-5">
        <div className="flex items-center gap-2 flex-wrap">
          <b className="text-[15px]">البيانات التجريبية</b>
          {st.has_demo ? <Badge c="b-amber">توجد بيانات تجريبية ({total} سجل)</Badge> : <Badge c="b-green">لا توجد بيانات تجريبية — النظام إنتاجي</Badge>}
          {st.seeded_at && <span className="text-[12px] num" style={{ color: 'var(--ink3)' }}>تاريخ البذر: {fmtDT(st.seeded_at)}</span>}
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-3">
          {st.tables.map(t => (
            <div key={t.table} className="rounded-xl p-2.5 text-center" style={{ background: t.demo ? 'rgba(217,119,6,.08)' : 'var(--card2)' }}>
              <div className="text-[12px] font-bold">{TBL_AR[t.table] || t.table}</div>
              <div className="text-[11.5px] num" style={{ color: 'var(--ink2)' }}>تجريبي: <b>{t.demo}</b> — إنتاجي: <b>{t.production}</b></div>
            </div>
          ))}
        </div>
        <div className="rounded-xl p-3.5 mt-3 text-[12.5px] leading-7" style={{ background: 'rgba(29,97,245,.06)', color: 'var(--ink2)' }}>
          الحذف الذكي يحذف السجلات التجريبية فقط بالترتيب الآمن (الدفعات ← المبيعات ← الحجوزات ← الوحدات ← العملاء...) ويحذف ملفاتها من القرص. الحسابات والأدوار والإعدادات وسجل التدقيق <b>لا تُمس أبدًا</b>.
        </div>
        <div className="flex gap-2 mt-3">
          <Btn v="d" disabled={!st.has_demo} onClick={() => setMode('delete')}><I n="trash" s={15} /> حذف البيانات التجريبية</Btn>
          <Btn v="g" onClick={() => setMode('reset')}><I n="refresh" s={15} /> إعادة تعيين الديمو (حذف + بذر جديد)</Btn>
        </div>
      </div>
      <Modal open={!!mode} onClose={() => { setMode(null); setWord(''); }} title={mode === 'delete' ? 'حذف البيانات التجريبية' : 'إعادة تعيين الديمو'} w={460}
        actions={<><Btn v={mode === 'delete' ? 'd' : 'p'} disabled={word !== (mode === 'delete' ? 'DELETE' : 'RESET') || busy} onClick={run}>{busy ? 'جارٍ التنفيذ...' : 'تأكيد نهائي'}</Btn><Btn v="g" onClick={() => { setMode(null); setWord(''); }}>تراجع</Btn></>}>
        <p className="text-[13.5px] leading-7">سيتم {mode === 'delete' ? `حذف ${total} سجلًا تجريبيًا نهائيًا` : 'حذف الديمو الحالي وبذر مجموعة تجريبية جديدة بتواريخ اليوم'}.</p>
        <p className="text-[13px] mt-2">للتأكيد اكتب <b className="num" dir="ltr">{mode === 'delete' ? 'DELETE' : 'RESET'}</b>:</p>
        <input className="inp num mt-2" dir="ltr" value={word} onChange={e => setWord(e.target.value)} placeholder={mode === 'delete' ? 'DELETE' : 'RESET'} />
      </Modal>
    </div>
  );
}
