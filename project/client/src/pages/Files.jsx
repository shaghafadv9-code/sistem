...
import { api, q, downloadFile, previewFileUrl, getToken } from '../lib/api.js';
...
export default function Files() {
  ...
  // رابط المعاينة يُجلب عبر تذكرة مؤقتة موقّعة (لا يُمرَّر رمز الدخول في الرابط)
  useEffect(() => {
    let alive = true;
    setPreviewUrl('');
    if (preview?.id) previewFileUrl(preview.id).then(u => { if (alive) setPreviewUrl(u); }).catch(() => {});
    return () => { alive = false; };
  }, [preview?.id]);
        ...
        {rows.map(f => (
          <div key={f.id} className="card card-h p-4">
            ...
            <div className="flex gap-1.5 mt-3">
              ...
              <Btn v="g" size="xs" onClick={() => downloadFile(f.id, f.original_name).catch(e => toast(e.message, 'error'))}><I n="dl" s={13} /> تنزيل</Btn>
      ...
      <Modal open={!!preview} onClose={() => setPreview(null)} title={preview?.original_name} w={760}>
        {preview && (
          preview.mime?.startsWith('image/')
            ? (previewUrl ? <img src={previewUrl} alt="" className="max-h-[60vh] mx-auto rounded-xl" /> : <Skeleton h={200} />)
            : preview.mime?.includes('pdf')
              ? (previewUrl ? <iframe src={previewUrl} className="w-full rounded-xl" style={{ height: '60vh' }} title="pdf" sandbox="allow-same-origin" /> : <Skeleton h={200} />)
              : <div className="text-center py-8"><p className="mb-4 text-[13.5px]" style={{ color: 'var(--ink2)' }}>لا يمكن معاينة هذا النوع داخل النظام — يُنزَّل كمرفق لأسباب أمنية</p><Btn v="p" onClick={() => downloadFile(preview.id, preview.original_name).catch(e => toast(e.message, 'error'))}><I n="dl" s={15} /> تنزيل الملف</Btn></div>
        )}
      </Modal>
