...
export default function Projects() {
  ...
  const saveUnit = async (f) => {
    ...
    try {
      ...
      toast('تم حفظ الوحدة'); setEditU(null); loadUnits(sel?.id); load(); refreshTree();
    } catch (e) { toast(e.message, 'error'); }
  };
    ...
    <>
      ...
      {!busy && !err && (sel || filters.project_scope === 'all') && (
        <div className="anim-in">
              ...
              {units.map(u => (
                <div key={u.id} className="card card-h p-4">
                  ...
                  <div className="flex gap-1.5 mt-3 flex-wrap">
                    ...
                    {can('units', 'edit') && <Btn v="g" size="sm" onClick={() => setEditU({ ...u, building_id: u.building_id || '', floor_id: u.floor_id || '' })}><I n="edit" s={14} /></Btn>}
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* هيكل المباني والأدوار */}
          {sel && tree && tree.buildings?.length > 0 && (
            <div className="card p-5 mt-5">
              <div className="flex items-center justify-between mb-2">
                <b className="text-[15px]">هيكل المشروع (المباني والأدوار)</b>
        ...
        {editU && (
          <div className="space-y-3">
            <div className="rounded-xl p-2.5 text-[12.5px]" style={{ background: 'var(--card2)', color: 'var(--ink2)' }}>
              التسلسل: المشروع ← المبنى ← الدور ← الوحدة. لا يمكن ربط الوحدة بدور لا يتبع المبنى المختار.
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label={`المبنى ${editU.require_building === 0 ? '(اختياري)' : ''}`}>
