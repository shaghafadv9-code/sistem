...
(async () => {
  ...
  // التحقق من توفر مستخدم سارة للاختبار
  let saraCheck = await req('/auth/login', { method: 'POST', body: { username: 'sara', password: 'sara1234' } });
  if (saraCheck.s !== 200) {
    const rolesRes = await req('/roles', {}, T);
    const secRole = (rolesRes.d?.data || []).find(r => r.name === 'secretary') || { id: 4 };
    await req('/users', { method: 'POST', body: { name: 'سارة السكرتيرة', username: 'sara', password: 'sara1234', role_id: secRole.id, status: 'active' } }, T);
    saraCheck = await req('/auth/login', { method: 'POST', body: { username: 'sara', password: 'sara1234' } });
  }
  const sara = saraCheck;
  ok('sara login', sara.s === 200);
  const ST = sara.d.token;
  const me = await req('/auth/me', {}, T);
  ok('me + perms', me.s === 200 && me.d.perms['tasks:create']);

  // التأكد من وجود حساب مالي فعال للنظام
  const accs = await req('/accounts', {}, T);
  if (!accs.d?.data || accs.d.data.length === 0) {
    await req('/accounts', { method: 'POST', body: { name: 'الصندوق الرئيسي', type: 'cash', opening_balance: 0, is_default: 1 } }, T);
  }
  ...
  // مهام + حالات
  const t1 = await req('/tasks', { method: 'POST', body: { title: 'مهمة QA', priority: 'high', status: 'new', client_id: cid, assignee_id: 1 } }, T);
  ...
  // مشاريع ووحدات
  let projs = await req('/projects?limit=20', {}, T);
  if (!projs.d?.data || projs.d.data.length < 4) {
    const defaultProjs = ['مشروع 101', 'مشروع 102', 'مشروع 103', 'مشروع 104'];
    for (const pName of defaultProjs) {
      if (!projs.d?.data?.some(p => p.name === pName)) {
        await req('/projects', { method: 'POST', body: { name: pName, city: 'الرياض', status: 'active', require_building: 0, require_floor: 0 } }, T);
      }
    }
    projs = await req('/projects?limit=20', {}, T);
  }
  for (const p of (projs.d?.data || [])) {
    if (p.require_building || p.require_floor) {
      await req('/projects/' + p.id, { method: 'PUT', body: { require_building: 0, require_floor: 0 } }, T);
    }
  }
  ok('projects 101-104 exist', projs.s === 200 && projs.d.data.length >= 4);

  let units = await req('/units?limit=5&status=available', {}, T);
  if (!units.d?.data || units.d.data.length === 0) {
    const pId = projs.d.data[0].id;
    await req('/units', { method: 'POST', body: { project_id: pId, type: 'شقة', rooms: 3, bathrooms: 2, price: 500000, status: 'available' } }, T);
    units = await req('/units?limit=5&status=available', {}, T);
  }
  ...
  // مساعد ذكي
  const oldAsTasks = await req('/tasks?limit=100', {}, T);
  for (const t of (oldAsTasks.d?.data || [])) {
    if (t.title === 'اختبار المساعد') await req('/tasks/' + t.id, { method: 'DELETE' }, T);
  }
