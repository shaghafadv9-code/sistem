...
function createQuotation(req, b = {}) {
  ...
  const code = `Q-${new Date().getFullYear()}-${String(db.prepare('SELECT COALESCE(MAX(id),0)+1 n FROM quotations').get().n).padStart(4, '0')}`;
...
function createContract(req, b = {}) {
  ...
  const code = `CON-${new Date().getFullYear()}-${String(db.prepare('SELECT COALESCE(MAX(id),0)+1 n FROM contracts').get().n).padStart(4, '0')}`;
...
function createSale(req, b = {}) {
  ...
  if (reservation_id) {
    ...
    if (!resv || resv.status !== 'active') throw Object.assign(new Error('الحجز غير نشط'), { code: 400 });
    ...
    unit = unitRow(unit_id);
    if (!unit) throw Object.assign(new Error('الوحدة غير موجودة'), { code: 404 });
    if (!['available', 'resale', 'reserved'].includes(unit.status)) throw Object.assign(new Error(`الوحدة غير متاحة للبيع (حالتها ${unit.status})`), { code: 400 });
  }
