...
const { db, flush, DATA_DIR, DB_PATH, seedDemo } = require('./db');
...
R.post('/brokers', auth, P('brokers', 'create'), need('name'), (req, res) => {
  ...
  const code = `B-${String(db.prepare('SELECT COALESCE(MAX(id),0)+1 n FROM brokers').get().n).padStart(4, '0')}`;
...
// ================= إدارة الديمو (مدير فقط) =================
const DEMO_TABLES = ['files', 'payments', 'invoices', 'sales', 'reservations', 'units', 'floors', 'buildings', 'calls', 'notes', 'tasks', 'appointments', 'projects', 'clients', 'brokers'];
R.get('/admin/demo-stats', auth, reqAdmin, (req, res) => {
  const tables = DEMO_TABLES.filter(t => { try { db.prepare(`SELECT is_demo FROM ${t} LIMIT 1`).get(); return true; } catch { return true; } }).map(t => {
    let demo = 0, total = 0;
    try { demo = db.prepare(`SELECT COUNT(*) c FROM ${t} WHERE is_demo=1`).get().c; total = db.prepare(`SELECT COUNT(*) c FROM ${t}`).get().c; } catch {}
    return { table: t, demo, total, production: total - demo };
  });
  res.json({ seeded_at: getSetting('demo_seeded_at', ''), tables, has_demo: tables.some(t => t.demo > 0) });
});
...
function demoBlockers() {
  ...
  chk('sales', 'مبيعات', `SELECT COUNT(*) c FROM sales s WHERE COALESCE(s.is_demo,0)=0 AND (s.unit_id IN (SELECT id FROM units WHERE is_demo=1) OR s.client_id IN (SELECT id FROM clients WHERE is_demo=1) OR s.reservation_id IN (SELECT id FROM reservations WHERE is_demo=1) OR s.broker_id IN (SELECT id FROM brokers WHERE is_demo=1))`);
  chk('reservations', 'حجوزات', `SELECT COUNT(*) c FROM reservations r WHERE COALESCE(r.is_demo,0)=0 AND (r.unit_id IN (SELECT id FROM units WHERE is_demo=1) OR r.client_id IN (SELECT id FROM clients WHERE is_demo=1))`);
  chk('payments', 'دفعات', `SELECT COUNT(*) c FROM payments p WHERE COALESCE(p.is_demo,0)=0 AND p.sale_id IN (SELECT id FROM sales WHERE is_demo=1)`);
  chk('invoices', 'فواتير', `SELECT COUNT(*) c FROM invoices i WHERE COALESCE(i.is_demo,0)=0 AND (i.sale_id IN (SELECT id FROM sales WHERE is_demo=1) OR i.client_id IN (SELECT id FROM clients WHERE is_demo=1))`);
  chk('files', 'ملفات', `SELECT COUNT(*) c FROM files f WHERE COALESCE(f.is_demo,0)=0 AND (f.client_id IN (SELECT id FROM clients WHERE is_demo=1) OR f.unit_id IN (SELECT id FROM units WHERE is_demo=1) OR f.project_id IN (SELECT id FROM projects WHERE is_demo=1) OR f.sale_id IN (SELECT id FROM sales WHERE is_demo=1) OR f.reservation_id IN (SELECT id FROM reservations WHERE is_demo=1) OR f.task_id IN (SELECT id FROM tasks WHERE is_demo=1) OR f.appointment_id IN (SELECT id FROM appointments WHERE is_demo=1))`);
  chk('tasks', 'مهام', `SELECT COUNT(*) c FROM tasks t WHERE COALESCE(t.is_demo,0)=0 AND (t.client_id IN (SELECT id FROM clients WHERE is_demo=1) OR t.project_id IN (SELECT id FROM projects WHERE is_demo=1))`);
  chk('appointments', 'مواعيد', `SELECT COUNT(*) c FROM appointments a WHERE COALESCE(a.is_demo,0)=0 AND a.client_id IN (SELECT id FROM clients WHERE is_demo=1)`);
  chk('calls', 'اتصالات', `SELECT COUNT(*) c FROM calls x WHERE COALESCE(x.is_demo,0)=0 AND x.client_id IN (SELECT id FROM clients WHERE is_demo=1)`);
  chk('notes', 'ملاحظات', `SELECT COUNT(*) c FROM notes n WHERE COALESCE(n.is_demo,0)=0 AND (n.client_id IN (SELECT id FROM clients WHERE is_demo=1) OR n.task_id IN (SELECT id FROM tasks WHERE is_demo=1) OR n.appointment_id IN (SELECT id FROM appointments WHERE is_demo=1))`);
  chk('units', 'وحدات', `SELECT COUNT(*) c FROM units u WHERE COALESCE(u.is_demo,0)=0 AND u.project_id IN (SELECT id FROM projects WHERE is_demo=1)`);
  return out;
}
function deleteDemoRows() {
  ...
  for (const t of DEMO_TABLES) {
    try {
      if (t === 'units') { counts[t] = db.prepare('DELETE FROM units WHERE is_demo=1').run().changes; continue; }
      if (t === 'buildings') { counts[t] = db.prepare('DELETE FROM buildings WHERE is_demo=1').run().changes; continue; }
...
R.post('/admin/demo-delete', auth, reqAdmin, (req, res) => {
  if (req.body?.confirm !== 'DELETE') return res.status(400).json({ error: 'التأكيد مطلوب' });
  const blockers = demoBlockers();
  if (blockers.length) return res.status(400).json({ error: 'تعذر الحذف — توجد سجلات إنتاجية مرتبطة بالبيانات التجريبية: ' + blockers.map(b => `${b.ar}: ${b.count}`).join('، ') + ' — احذفها أو انقلها أولًا', blockers });
  const counts = deleteDemoRows();
  const n = Object.values(counts).reduce((a, b) => a + b, 0);
  pushAudit(req, 'delete', 'settings', 'demo_data', null, `حذف البيانات التجريبية (${n} سجل)`);
  res.json({ ok: true, deleted: n, counts });
});
R.post('/admin/demo-reset', auth, reqAdmin, (req, res) => {
  if (req.body?.confirm !== 'RESET') return res.status(400).json({ error: 'التأكيد مطلوب' });
  const blockers = demoBlockers();
  if (blockers.length) return res.status(400).json({ error: 'تعذر التصفير — توجد سجلات إنتاجية مرتبطة بالديمو: ' + blockers.map(b => `${b.ar}: ${b.count}`).join('، '), blockers });
  deleteDemoRows();
  try { seedDemo(); } catch (e) { return res.status(500).json({ error: 'فشلت إعادة البذر: ' + e.message }); }
  pushAudit(req, 'create', 'settings', 'demo_data', null, 'إعادة تعيين البيانات التجريبية');
  res.json({ ok: true });
});
...
R.get('/followups', auth, (req, res) => {
  const p = req.user.perms, t = new Date().toISOString().slice(0, 10);
