...
R.post('/accounts', auth, P('accounts', 'create'), need('name'), (req, res) => {
  ...
  const code = `ACC-${String(db.prepare('SELECT COALESCE(MAX(id),0)+1 n FROM accounts').get().n).padStart(4, '0')}`;
...
R.post('/accounts/transfer', auth, P('accounts', 'manage'), (req, res) => {
  const { from_account_id, to_account_id, amount, txn_date, reference_no = '', notes = '' } = req.body || {};
  const amt = num(amount);
  if (!from_account_id || !to_account_id || from_account_id === to_account_id) return res.status(400).json({ error: 'حدد حسابين مختلفين' });
  if (amt <= 0) return res.status(400).json({ error: 'المبلغ يجب أن يكون أكبر من صفر' });
  const from = db.prepare('SELECT * FROM accounts WHERE id=? AND deleted_at IS NULL').get(from_account_id);
  const to = db.prepare('SELECT * FROM accounts WHERE id=? AND deleted_at IS NULL').get(to_account_id);
  if (!from || !to) return res.status(404).json({ error: 'أحد الحسابين غير موجود' });
  const id = F.postTxn({ kind: 'transfer', direction: 'out', amount: amt, accountId: from.id, toAccountId: to.id, txnDate: txn_date || today(), method: 'transfer', reference: reference_no, notes: notes || `تحويل من ${from.name} إلى ${to.name}`, user: req.user });
  push(req, 'create', 'accounts', 'transfer', id, `${amt} — ${from.name} → ${to.name}`);
  res.status(201).json({ ok: true, txn_id: id, from_balance: F.recomputeAccount(from.id), to_balance: F.recomputeAccount(to.id) });
});
...
R.post('/transactions/:id/reverse', auth, P('finance', 'manage'), idParam, (req, res) => {
  ...
  if (t.reversed_by || t.kind === 'reversal') return res.status(400).json({ error: 'الحركة معكوسة مسبقًا' });
  const reason = (req.body || {}).reason;
  if (!reason) return res.status(400).json({ error: 'سبب العكس مطلوب' });
  const id = F.postTxn({
    kind: 'reversal', direction: t.direction === 'in' ? 'out' : 'in', amount: t.amount, accountId: t.account_id, txnDate: today(),
    method: t.method, reference: t.reference_no, clientId: t.client_id, projectId: t.project_id, unitId: t.unit_id, saleId: t.sale_id,
    reservationId: t.reservation_id, notes: `عكس حركة #${t.id}: ${reason}`, user: req.user
  });
  db.prepare('UPDATE transactions SET status=\'reversed\', reversed_by=? WHERE id=?').run(id, t.id);
  db.prepare('UPDATE transactions SET reversal_of=? WHERE id=?').run(t.id, id);
  if (t.kind !== 'reversal' && t.sale_id) F.applyPaymentToSchedule(t.sale_id);
  push(req, 'update', 'finance', 'reversal', t.id, reason);
  res.json({ ok: true, reversal_id: id });
});
...
R.put('/payments/:id', auth, P('finance', 'edit'), idParam, (req, res) => {
  ...
  const b = req.body || {};
  const doEdit = (user) => {
    const amount = b.amount !== undefined ? num(b.amount) : num(p.amount);
    if (amount <= 0) throw Object.assign(new Error('المبلغ غير صالح'), { code: 400 });
    const paid_at = b.paid_at || p.paid_at;
    const method = b.method || p.method;
    const account_id = b.account_id !== undefined ? b.account_id : p.account_id;
    const cols = ['amount', 'method', 'account_id', 'reference_no', 'check_no', 'check_date', 'check_due_date', 'check_status', 'bank_name', 'paid_at', 'notes', 'attachment_id', 'kind'];
    const vals = { amount, method, account_id, reference_no: b.reference_no ?? p.reference_no, check_no: b.check_no ?? p.check_no, check_date: b.check_date ?? p.check_date, check_due_date: b.check_due_date ?? p.check_due_date, check_status: b.check_status ?? p.check_status, bank_name: b.bank_name ?? p.bank_name, paid_at, notes: b.notes ?? p.notes, attachment_id: b.attachment_id ?? p.attachment_id, kind: b.kind || p.kind };
    db.prepare(`UPDATE payments SET ${cols.map(c => `${c}=?`).join(',')}, updated_at=datetime('now','localtime') WHERE id=?`).run(...cols.map(c => vals[c]), p.id);
    if (p.txn_id) {
      db.prepare(`UPDATE transactions SET amount=?, method=?, account_id=?, reference_no=?, check_no=?, check_date=?, check_due_date=?, check_status=?, bank_name=?, txn_date=?, notes=?, status=? WHERE id=?`)
        .run(amount, method, account_id, vals.reference_no, vals.check_no, vals.check_date, vals.check_due_date, vals.check_status, vals.bank_name, paid_at, `تعديل دفعة #${p.id}`, p.status === 'confirmed' ? 'confirmed' : 'pending', p.txn_id);
      F.recomputeAccount(p.account_id); if (account_id && account_id !== p.account_id) F.recomputeAccount(account_id);
    }
    if (p.sale_id) { F.applyPaymentToSchedule(p.sale_id); db.prepare('UPDATE sales SET net_paid_cache=? WHERE id=?').run(F.saleTotals(p.sale_id).paid, p.sale_id); }
    F.pushAudit(user, 'update', 'finance', 'payment', p.id, `تعديل دفعة — المبلغ ${amount}`);
    return { ok: true };
  };
  if (!F.canApprove(req.user)) {
    const g = F.gate(req, { action_type: 'payment_edit', entity_type: 'payment', entity_id: p.id, title: `تعديل دفعة #${p.id} بمبلغ ${num(p.amount)}`, module: 'finance', amount: num(p.amount), payload: { payment_id: p.id, ...b } });
...
R.post('/payments/:id/cancel', auth, P('finance', 'edit'), idParam, (req, res) => {
  ...
  const doCancel = (user) => {
    db.prepare(`UPDATE payments SET status='cancelled', deleted_at=datetime('now','localtime'), notes=COALESCE(NULLIF(notes,''),'') || ' — ملغاة: ' || ?, updated_at=datetime('now','localtime') WHERE id=?`).run(reason, p.id);
    if (p.txn_id) {
      F.postTxn({ kind: 'reversal', direction: 'out', amount: p.amount, accountId: p.account_id, txnDate: today(), method: p.method, reference: p.reference_no, clientId: p.client_id, projectId: p.project_id, unitId: p.unit_id, saleId: p.sale_id, reservationId: p.reservation_id, notes: `إلغاء دفعة #${p.id}: ${reason}`, user });
      db.prepare(`UPDATE transactions SET status='reversed', reversed_by=(SELECT MAX(id) FROM transactions) WHERE id=?`).run(p.txn_id);
    }
    if (p.sale_id) { F.applyPaymentToSchedule(p.sale_id); db.prepare('UPDATE sales SET net_paid_cache=? WHERE id=?').run(F.saleTotals(p.sale_id).paid, p.sale_id); }
    F.pushAudit(user, 'update', 'finance', 'payment_cancel', p.id, reason);
    return { ok: true };
  };
  if (!F.canApprove(req.user)) {
...
R.post('/payments/:id/check-status', auth, P('finance', 'edit'), idParam, (req, res) => {
  ...
  const st = (req.body || {}).check_status;
  if (!['pending', 'cleared', 'bounced', 'cancelled'].includes(st)) return res.status(400).json({ error: 'حالة شيك غير صالحة' });
  const payStatus = st === 'cleared' ? 'confirmed' : st === 'bounced' ? 'bounced' : 'pending';
  db.prepare(`UPDATE payments SET check_status=?, status=?, updated_at=datetime('now','localtime') WHERE id=?`).run(st, payStatus, p.id);
  if (p.txn_id) db.prepare('UPDATE transactions SET check_status=?, status=? WHERE id=?').run(st, payStatus === 'confirmed' ? 'cleared' : payStatus === 'bounced' ? 'bounced' : 'pending', p.txn_id);
  if (p.sale_id) { F.applyPaymentToSchedule(p.sale_id); }
  push(req, 'update', 'finance', 'check_status', p.id, `${p.check_no}: ${st}`);
  res.json({ ok: true, status: payStatus });
});
...
R.post('/expense-categories', auth, P('expenses', 'create'), need('name'), (req, res) => {
  const code = `EXC-${String(db.prepare('SELECT COALESCE(MAX(id),0)+1 n FROM expense_categories').get().n).padStart(2, '0')}`;
...
R.post('/expenses', auth, P('expenses', 'create'), need('amount'), (req, res) => {
  ...
  const code = `EXP-${new Date().getFullYear()}-${String(db.prepare('SELECT COALESCE(MAX(id),0)+1 n FROM expenses').get().n).padStart(4, '0')}`;
...
R.post('/expenses/:id/cancel', auth, P('expenses', 'edit'), idParam, (req, res) => {
  ...
  if (e.status === 'paid') {
    const doCancel = (user) => {
      if (e.txn_id) {
        const t = db.prepare('SELECT * FROM transactions WHERE id=?').get(e.txn_id);
        const rid = F.postTxn({ kind: 'reversal', direction: 'in', amount: t.amount, accountId: t.account_id, txnDate: today(), method: t.method, notes: `إلغاء مصروف ${e.code}: ${reason}`, user });
        db.prepare(`UPDATE transactions SET status='reversed', reversed_by=? WHERE id=?`).run(rid, t.id);
        db.prepare('UPDATE transactions SET reversal_of=? WHERE id=?').run(t.id, rid);
      }
...
R.post('/refunds', auth, P('finance', 'create'), need('client_id'), (req, res) => {
  ...
  const code = `REF-${new Date().getFullYear()}-${String(db.prepare('SELECT COALESCE(MAX(id),0)+1 n FROM refunds').get().n).padStart(4, '0')}`;
...
R.get('/commissions', auth, P('commissions', 'view'), (req, res) => {
  ...
  const rows = db.prepare(`SELECT s.id, s.code sale_code, s.sale_date, s.net_price, s.commission, b.id broker_id, b.name broker_name, b.commission_rate, b.commission_method, b.iban, b.due_days,
    ...
    const d = new Date((r0.sale_date || today()) + 'T00:00:00'); d.setDate(d.getDate() + dueDays);
...
R.post('/commissions/payout', auth, P('commissions', 'create'), need('broker_id', 'amount'), (req, res) => {
  ...
  const accId = b.account_id || F.defaultAccountId();
  const code = `CMP-${new Date().getFullYear()}-${String(db.prepare('SELECT COALESCE(MAX(id),0)+1 n FROM commission_payments').get().n).padStart(4, '0')}`;
...
F.registerApprovalExecutor('payment_edit', (p, approverUser) => {
  ...
  const amount = p.amount !== undefined ? num(p.amount) : num(pay.amount);
  const paid_at = p.paid_at || pay.paid_at, method = p.method || pay.method;
  const account_id = p.account_id !== undefined ? p.account_id : pay.account_id;
  db.prepare(`UPDATE payments SET amount=?, method=?, account_id=?, reference_no=?, check_no=?, check_date=?, check_due_date=?, check_status=?, bank_name=?, paid_at=?, notes=?, kind=?, updated_at=datetime('now','localtime') WHERE id=?`)
    .run(amount, method, account_id, p.reference_no ?? pay.reference_no, p.check_no ?? pay.check_no, p.check_date ?? pay.check_date, p.check_due_date ?? pay.check_due_date,
      p.check_status ?? pay.check_status, p.bank_name ?? pay.bank_name, paid_at, p.notes ?? pay.notes, p.kind || pay.kind, pay.id);
  if (pay.txn_id) {
    db.prepare(`UPDATE transactions SET amount=?, method=?, account_id=?, reference_no=?, check_no=?, paid_at=paid_at, txn_date=? WHERE id=?`).run(amount, method, account_id, p.reference_no ?? pay.reference_no, p.check_no ?? pay.check_no, paid_at, pay.txn_id);
    F.recomputeAccount(account_id); F.recomputeAccount(pay.account_id);
  }
  if (pay.sale_id) F.applyPaymentToSchedule(pay.sale_id);
  return { note: 'تم تعديل الدفعة بعد الاعتماد' };
});
F.registerApprovalExecutor('payment_cancel', (p, approverUser) => {
  const pay = db.prepare('SELECT * FROM payments WHERE id=?').get(p.payment_id);
  if (!pay) throw Object.assign(new Error('الدفعة غير موجودة'), { code: 404 });
  db.prepare(`UPDATE payments SET status='cancelled', deleted_at=datetime('now','localtime'), updated_at=datetime('now','localtime') WHERE id=?`).run(pay.id);
  if (pay.txn_id) {
    const rid = F.postTxn({ kind: 'reversal', direction: 'out', amount: pay.amount, accountId: pay.account_id, txnDate: today(), method: pay.method, notes: `إلغاء دفعة #${pay.id} (اعتماد): ${p.reason || ''}`, saleId: pay.sale_id, reservationId: pay.reservation_id, clientId: pay.client_id, user: approverUser });
    db.prepare('UPDATE transactions SET status=\'reversed\', reversed_by=? WHERE id=?').run(rid, pay.txn_id);
    db.prepare('UPDATE transactions SET reversal_of=? WHERE id=?').run(pay.txn_id, rid);
  }
  if (pay.sale_id) { F.applyPaymentToSchedule(pay.sale_id); db.prepare('UPDATE sales SET net_paid_cache=? WHERE id=?').run(F.saleTotals(pay.sale_id).paid, pay.sale_id); }
...
F.registerApprovalExecutor('expense_cancel', (p, approverUser) => {
  ...
  if (e.txn_id) {
    const t = db.prepare('SELECT * FROM transactions WHERE id=?').get(e.txn_id);
    const rid = F.postTxn({ kind: 'reversal', direction: 'in', amount: t.amount, accountId: t.account_id, txnDate: today(), notes: `إلغاء مصروف ${e.code} (اعتماد): ${p.reason || ''}`, user: approverUser });
    db.prepare('UPDATE transactions SET status=\'reversed\', reversed_by=? WHERE id=?').run(rid, t.id);
    db.prepare('UPDATE transactions SET reversal_of=? WHERE id=?').run(t.id, rid);
  }
