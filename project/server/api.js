...
const { db, nextUniqueCode, MODULES, ACTIONS, DATA_DIR, DB_PATH } = require('./db');
const { auth, requirePerm, signToken, audit, can } = require('./auth');
const { loginLimiter, validatePassword } = require('./security');
...
function nextCode(prefix, table, col = 'code') {
  return nextUniqueCode(prefix, table, col);
}
...
function resource({ table, module, fields, search = [], select = null, order = 'id', enrich = null, extraWhere = null, before = null, after = null, whereBase = null, orderBy = null, excludeKeys = [] }) {
  ...
  r.post('/', auth, P(module, 'create'), (req, res) => {
    try {
      // توليد الأكواد تلقائياً من الباك إند إذا لم يُدخل المستخدم كوداً
      if (fields.includes('code') && (!req.body.code || !String(req.body.code).trim())) {
        const pfx = table === 'projects' ? 'PROJECT' : table === 'units' ? 'UNIT' : table === 'clients' ? 'CUSTOMER' : table.toUpperCase().slice(0, 8);
        req.body.code = nextUniqueCode(pfx, table, 'code');
      }
  ...
  r.put('/:id', auth, P(module, 'edit'), (req, res) => {
    try {
      // تعديل المشاريع والوحدات محصور بمدير النظام (Admin) فقط
      if ((table === 'projects' || table === 'units') && req.user.role !== 'admin') {
        audit(req.user, 'denied', module, table, req.params.id, 'محاولة تعديل مرفوضة — للأدمن فقط');
        return res.status(403).json({ error: 'صلاحية مرفوضة: تعديل ' + (table === 'projects' ? 'المشاريع' : 'الوحدات') + ' متاح لمدير النظام (Admin) فقط' });
      }
  ...
  r.delete('/:id', auth, P(module, 'delete'), (req, res) => {
    // حذف المشاريع والوحدات محصور بمدير النظام (Admin) فقط
    if ((table === 'projects' || table === 'units') && req.user.role !== 'admin') {
      audit(req.user, 'denied', module, table, req.params.id, 'محاولة حذف مرفوضة — للأدمن فقط');
      return res.status(403).json({ error: 'صلاحية مرفوضة: حذف ' + (table === 'projects' ? 'المشاريع' : 'الوحدات') + ' متاح لمدير النظام (Admin) فقط' });
    }
    const cur = db.prepare(`SELECT * FROM ${table} WHERE id=? AND deleted_at IS NULL`).get(req.params.id);
    if (!cur) return res.status(404).json({ error: 'السجل غير موجود' });
...
R.get('/auth/me', auth, (req, res) => {
  ...
  let mustChange = 0;
  try { mustChange = Number(db.prepare('SELECT COALESCE(must_change_password,0) m FROM users WHERE id=?').get(u.id)?.m || 0); } catch {}
  res.json({ user: { ...u, must_change_password: mustChange }, perms, must_change_password: !!mustChange });
});
R.post('/auth/change-password', auth, (req, res) => {
  const { current, next } = req.body || {};
  if (!current || !next) return res.status(400).json({ error: 'أدخل كلمة المرور الحالية والجديدة' });
  const u = db.prepare('SELECT * FROM users WHERE id=?').get(req.user.id);
  const pwCheck = validatePassword(next, { username: u.username });
  if (!pwCheck.ok) return res.status(400).json({ error: pwCheck.error });
  if (current === next) return res.status(400).json({ error: 'كلمة المرور الجديدة يجب أن تختلف عن الحالية' });
  if (!bcrypt.compareSync(current, u.password_hash)) return res.status(400).json({ error: 'كلمة المرور الحالية غير صحيحة' });
  db.prepare("UPDATE users SET password_hash=?, must_change_password=0, updated_at=datetime('now','localtime') WHERE id=?").run(bcrypt.hashSync(next, 10), u.id);
  // حذف ملف كلمة المرور الأولية بعد تغييرها
  try { const f = require('path').join(DATA_DIR, '.initial-admin-password'); if (require('fs').existsSync(f)) require('fs').unlinkSync(f); } catch {}
...
R.post('/users', auth, P('users', 'create'), (req, res) => {
  ...
  const pwCheck = validatePassword(password, { username });
  if (!pwCheck.ok) return res.status(400).json({ error: pwCheck.error });
  try {
...
R.use('/clients', resource({
  ...
  before: (body, req, cur) => {
    if (!cur) {
      ...
      if (!body.code) {
        body.code = nextUniqueCode('CUSTOMER', 'clients', 'code');
      }
...
R.use('/projects', resource({
  ...
  fields: ['code', 'name', 'city', 'address', 'description', 'status', 'require_building', 'require_floor', 'default_floor_type'],
  ...
  select: `SELECT projects.*, (SELECT COUNT(*) FROM units WHERE units.project_id=projects.id) units_count,
    ...
    (SELECT COUNT(*) FROM units WHERE units.project_id=projects.id AND units.status='reserved') reserved_count,
    (SELECT COUNT(*) FROM units WHERE units.project_id=projects.id AND units.status='sold') sold_count FROM projects`,
  // Project → Units: المبنى/الدور اختياريان افتراضيًا (يُطلبان فقط إذا فعّلهما المستخدم صراحةً للمشروع)
  before: (body, req, cur) => {
    if (!cur) {
      if (body.require_building === undefined || body.require_building === null || body.require_building === '') body.require_building = 0;
      if (body.require_floor === undefined || body.require_floor === null || body.require_floor === '') body.require_floor = 0;
    }
    if (body.require_building !== undefined) body.require_building = Number(body.require_building) === 1 ? 1 : 0;
    if (body.require_floor !== undefined) body.require_floor = Number(body.require_floor) === 1 ? 1 : 0;
    return null;
  }
}));
