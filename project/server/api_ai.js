...
R.get('/status', auth, P('ai', 'view'), (req, res) => {
  ...
  // لا يُعلن النظام أن الذكاء الاصطناعي يعمل إذا كانت كل النماذج فاشلة
  const verdict = h.verdict;
  res.json({
    verdict, operational: h.operational,
    providers_enabled: providers,
    models: { total: h.totals.models, healthy: h.totals.healthy, failed: h.totals.failed, degraded: h.totals.degraded, unknown: h.totals.unknown },
    def: model ? { id: model.id, model: model.model_id, alias: model.alias, provider: model.pname, status: model.last_status, fallback } : null,
    limits: h.limits,
    last_healthy_model_id: h.last_healthy_model_id,
    admin_default_model_id: h.admin_default_model_id,
    usage_today: { calls: today.c, tokens: today.t },
    hint: verdict === 'FAILED' ? 'كل النماذج في حالة فشل — راجع المفاتيح والاتصال من صفحة الذكاء الاصطناعي' : verdict === 'NOT_CONFIGURED' ? 'لم يُضبط أي مزود/نموذج بعد' : ''
  });
});
// حالة مفصّلة لكل نموذج (تشخيص سريع للمستخدم)
R.get('/health', auth, P('ai', 'view'), (req, res) => res.json(AI.aiHealth()));
...
function assistantContext(user) {
  ...
  const ctx = { date: require('./time').today(), user: user.name, role: user.role_ar || user.role, modules: [] };
...
// سياسة موحدة: عرض صلاحيات AI يحتاج ai:view أو ai:manage (أو مدير النظام)
const requireAIView = (req, res, next) => {
  const isManager = req.user.role === 'admin' || !!req.user.perms['ai:manage'];
  if (!isManager && !req.user.perms['ai:view']) return res.status(403).json({ error: 'لا تملك صلاحية عرض صلاحيات الذكاء الاصطناعي' });
  next();
};
R.get('/permissions', auth, requireAIView, (req, res) => {
  try {
    seedAIPermissions();
...
R.put('/permissions', auth, (req, res) => {
  // إدارة صلاحيات الذكاء الاصطناعي = ai:manage أو مدير النظام فقط.
  // roles:manage لم تعد تمنح صلاحية AI تلقائيًا (فصل الصلاحيات).
  const isManager = req.user.role === 'admin' || req.user.perms['ai:manage'];
  if (!isManager) {
    return res.status(403).json({ error: 'صلاحية إدارة صلاحيات الذكاء الاصطناعي مقتصرة على ai:manage أو مدير النظام' });
  }
