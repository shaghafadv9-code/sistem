...
// مُوحَّد عبر ai_permissions.resolveAIPermission — نفس القواعد في كل طبقات الذكاء الاصطناعي.
function checkDualPermission(module, action, user) {
  const r = AIP.resolveAIPermission(module, action, user);
  return { allowed: r.allowed, reply: r.reply, reason: r.reason, scope: r.scope, resolved: r.resolved };
}
  ...
  if (/(?:أضف|اضف|انشئ|أنشئ|سجل|جدول|احجز|سو|اعمل|حط)\s*(?:لي)?\s*موعد/i.test(userText)) {
    ...
    if (!time) {
      context.pending_flow = {
        ...
        data: { client, date: date || TT.today(), title },
        waiting_for: 'time'
      };
      return respond({
