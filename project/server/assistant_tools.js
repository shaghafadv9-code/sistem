  ...
  changeTaskStatus: {
    ...
    execute: async (params, user) => {
      ...
      if (params.status === 'completed') {
        compAt = new Date().toISOString().slice(0, 19).replace('T', ' ');
      }
...
async function executeAssistantTool(toolName, params, user, { confirmed = false } = {}) {
  ...
  // 1. فحص الاستثناء الأمني المطلق للإعدادات والصلاحيات
  if (tool.module === 'settings' || ['settings', 'users', 'roles', 'backup', 'ai_permissions'].includes(tool.module)) {
    const err = 'لا أملك صلاحية الوصول إلى الإعدادات أو الصلاحيات أو البيانات الأمنية للنظام.';
    logAssistantAction({
      userId: user.id,
      username: user.username,
      actionType: toolName,
      inputParams: params,
      resultStatus: 'ai_security_denied',
      errorMessage: err
    });
    return { ok: false, denied: true, aiDenied: true, error: err };
  }

  // 2. فحص صلاحية الذكاء الاصطناعي (AI Permission Check)
  if (tool.module && tool.action) {
    const hasAIPerm = canAI(tool.module, tool.action);
    if (!hasAIPerm) {
      const modAr = MODULE_NAMES_AR[tool.module] || tool.module;
      const actAr = ACTION_NAMES_AR[tool.action] || tool.action;
      const err = `لا أملك صلاحية ${actAr} ${modAr} حالياً.`;
      logAssistantAction({
        userId: user.id,
        username: user.username,
        actionType: toolName,
        inputParams: params,
        resultStatus: 'ai_permission_denied',
        errorMessage: err
      });
      return { ok: false, denied: true, aiDenied: true, error: err };
    }
  ...
  // 3. فحص صلاحية المستخدم (User Permission Check)
  if (tool.module && tool.action) {
    const isAdmin = user && (user.role === 'admin' || user.role_id === 1 || user.username === 'admin');
    const hasPerm = isAdmin || !!user.perms?.['*:*'] || can(user.perms, tool.module, tool.action);
    if (!hasPerm) {
      const modAr = MODULE_NAMES_AR[tool.module] || tool.module;
      const actAr = ACTION_NAMES_AR[tool.action] || tool.action;
      const err = `⚠️ ليس لديك صلاحية ${actAr} على ${modAr}.`;
      logAssistantAction({
        userId: user.id,
        username: user.username,
        actionType: toolName,
        inputParams: params,
        resultStatus: 'user_permission_denied',
        errorMessage: err
      });
